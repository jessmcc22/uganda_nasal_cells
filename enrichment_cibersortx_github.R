...
res = read.csv("de_nasal.csv", header=TRUE)
head(res)
...
# read in your DEG dataset
res = read.csv("de_blood.csv", header=TRUE)
colnames(res)[1] <- "symbol"
res <- as_tibble(res)
head(res)
res2 <- res %>% 
...
# differential expression table
de_blood = read.csv("de_blood.csv", header=TRUE, row.names = 1)
...
genes <-read.delim("genes_annotation_final.txt", header = TRUE, sep = "\t", dec=".")
...
cibersort <- read.delim("CIBERSORTx_Job2_Adjusted.txt", header = TRUE, sep = "\t", dec = ".") 
...
cibersort <- read.delim("CIBERSORTx_Job3_Adjusted.txt", header = TRUE, sep = "\t", dec = ".") 
names(cibersort) <- gsub(x = names(cibersort), pattern = "\\.", replacement = "_")
row.names(cibersort) <- cibersort$Mixture
cibersort <- cibersort[,c(2:23)]
row_sums <- rowSums(cibersort)
